...
*
****************************************************************************************************************************************/
void EMPI_NoMonitor_init (int *rank, int procs_hint) {
    ...
    /* CHANGE: BEGIN */
    if (*rank == EMPI_root) {
        signal_thread_rpc_malleability_enter_region(procs_hint);
    }
    /* CHANGE: END */
...
void EMPI_Exec_Malleability  (int *rank, int *size, int count, int disp, EMPI_Monitor_type *smonitor, int *fc, char *argv[], char *bin, int newsize, int mflops, int **hostid_orig, int **rremvs_orig, int doLBalance) {
    ...
    if (newsize > *size) {
        ...
        /* CHANGE: begin */
        fprintf (stdout, "\n*** DEBUG_MSG::PMPI_Barrier(begin)::EMPI_Monitor_malleability_triggered in <%s> ***\n", __FILE__);
        PMPI_Barrier (EMPI_COMM_WORLD);
        fprintf (stdout, "\n*** DEBUG_MSG::PMPI_Barrier(end)::EMPI_Monitor_malleability_triggered in <%s> ***\n", __FILE__);
        /* CHANGE: end */

        //remove processes
        EMPI_Monitor_remove (rank, size, (*size-newsize), count, disp, smonitor, rremvs, fc);
        fprintf (stdout, "EMPI_Monitor_malleability_triggered: EMPI_Monitor_remove\n");
        
        /* CHANGE: begin */
        if (*rank == EMPI_root) {
            printf("EMPI_Monitor_malleability_triggered: sleep and call command_rpc_release_nodes \n");
            //for (int i=0;i<20;i++) {
            //    sleep(1);
            //    fprintf (stdout, "release_nodes: %d sec\n",i+1);
            //}
            // CHANGE TEMP
...
void EMPI_Monitor_end (int *rank, int *size, int iter, int maxiter, int *count, int *disp, int **vcount, int **vdisp, int *fc, char *argv[], char *bin) {
    ...
    if (EMPI_GLOBAL_mpolicy == EMPI_MONITORDBG) {
       ...
       if ((iter > 0)&&((iter % EMPI_GLOBAL_niter) == 0)) {
           ...
           printf ("EMPI_MONITOR_END: ITERACION %d\n",iter);
           // Increment the values of the cost struct
           EMPI_Inc_Cost_struct (*rank, MPI_Wtime(),0);
...
void EMPI_Monitor_init (int *rank, int procs_hint) {
/* CHANGE: end */
...
static void EMPI_Monitor_malleability_triggered (int *rank, int *size, int iter, int maxiter, int count, int disp, EMPI_Monitor_type *smonitor, int *fc, char *argv[], char *bin) {
    ...
    fprintf (stdout, "EMPI_Monitor_malleability_triggered: after if\n");
    ...
    fprintf (stdout, "EMPI_Monitor_malleability_triggered: before unlock if\n");
    pthread_mutex_unlock(&EMPI_GLOBAL_server_lock); // David global unlock
    fprintf (stdout, "EMPI_Monitor_malleability_triggered: after unlock if\n");
    ...
    */
    ...
    if (status == EMPI_ACTIVE) {
        ...
        */
        ...
        fprintf (stdout, "NOT Executing MPI_Finalize (%s)\n", __FILE__);
        ...
        *rank = MPI_PROC_NULL;
        ...
        //exit(0);
        /* CHANGE:  end */
        fprintf (stdout, "EMPI_Monitor_remove BEFORE MPI_Finalize (pid:%d),status:%d=%d\n", getpid(),EMPI_GLOBAL_status2,EMPI_REMOVED);
        pthread_mutex_unlock(&EMPI_GLOBAL_server_lock);
        //exit(0);
        //MPI_Finalize ();
        pthread_mutex_lock(&EMPI_GLOBAL_server_lock);
        fprintf (stdout, "EMPI_Monitor_remove AFTER MPI_Finalize (pid:%d),status:%d=%d\n", getpid(),EMPI_GLOBAL_status2,EMPI_REMOVED);

        fprintf (stdout, "END MPI_Finalize (%s)\n", __FILE__);

    }    

    free (vcounts);
    free (displs);
    fprintf (stdout, "END free (%s)\n", __FILE__);
    ...
    #endif
    fprintf (stdout, "END EMPI_Monitor_remove (%s)\n", __FILE__);
