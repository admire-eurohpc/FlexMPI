...
#define FLEXMPI_COMMAND_QUEUE_SIZE 20


struct command_flexmpi_queue {
    int    first_empty_pos;
    struct command_flexmpi  *cmd[FLEXMPI_COMMAND_QUEUE_SIZE];
};
...
int flexmpi_reconfigure(int shrink, uint32_t maxprocs, const char *hostlist, void *data) {
  ...
  int mod_procs = 0;
  struct command_flexmpi_queue *cmd_queue = data;
  struct command_flexmpi *cmd = data;
  ...
  bzero(command, FLEXMPI_COMMAND_LEN);
  snprintf(command, FLEXMPI_COMMAND_LEN, "6:%s", hostlist);

  printf("\nCommand = %s\n",command);

  if (cmd_queue->first_empty_pos == 0) {
     printf("Reconfigure function: cmd_queue is empty\n");
  } else {
    printf("Reconfigure function: cmd_queue content");
    for (int i=0; i<cmd_queue->first_empty_pos; i++) {
        printf(" => %d",cmd_queue->cmd[i]->command_n);
        int inc = 0;
        do {
          printf("::%s:%s", cmd_queue->cmd[i]->options[inc],cmd_queue->cmd[i]->options[inc+1]);
          inc = inc + 2;
        } while (cmd_queue->cmd[i]->options[inc] != NULL);
    }
    printf("\n");
  }
  if (command == NULL || strlen(command) == 0){
    ...
    return ICC_FAILURE;
      ...
      if (shrink == 0) {
        /* get and alloc the next position in queue */
        if (cmd_queue->first_empty_pos >= FLEXMPI_COMMAND_QUEUE_SIZE) {
          printf("Reconfigure function(grow): ERROR, cmd_queue full\n");
          return ICC_FAILURE;
        }
        cmd_queue->cmd[cmd_queue->first_empty_pos] = (struct command_flexmpi *)malloc(sizeof(struct command_flexmpi));
        cmd = cmd_queue->cmd[cmd_queue->first_empty_pos];
        cmd_queue->first_empty_pos++;
        bzero((char *)cmd, sizeof(struct command_flexmpi));
        free_cmd = 0;
        ...
        /* get and alloc the next position in queue */
        if (cmd_queue->first_empty_pos <= 0 ) {
          printf("Reconfigure function(shrink): ERROR, cmd_queue empty\n");
          return ICC_FAILURE;
        }
        cmd_queue->first_empty_pos--;
        cmd = cmd_queue->cmd[cmd_queue->first_empty_pos];
        ...
        printf("Command: %d", cmd->command_n);
        for (int i = 0; i<NUMBER_OPTIONS; i++) {
          if (cmd->options[i] == NULL) break;
          if ((i % 2) == 0) {
            printf(":%s", cmd->options[i]);
          } else {
            int aux = atoi(cmd->options[i]);
            free(cmd->options[i]);
            cmd->options[i] = (char *) malloc (10);
            bzero(cmd->options[i],10);
            sprintf(cmd->options[i], "%d", (-1 * aux));
          }
        }
      }
          ...
          case 6:
              ...
              if(EMPI_GLOBAL_listrm[0] >0){
                printf(" Command ignored: previous command already being processed. Try again later \n" );
                   ...
                   do
                   {
                      printf(" Command 6: proc_increment:%d, cmd->options[proc_increment]: %s,cmd->options[proc_increment+1]: %s \n",proc_increment,cmd->options[proc_increment],cmd->options[proc_increment+1]);
                      for (n_classes = 0; n_classes < EMPI_GLOBAL_nhclasses; n_classes++) {
                          printf(" Command 6: EMPI_GLOBAL_nprocs_class[0][n_classes]: %d,EMPI_GLOBAL_hclasses[n_classes]: %s \n",EMPI_GLOBAL_nprocs_class[0][n_classes],EMPI_GLOBAL_hclasses[n_classes]);
                          ...
                          if (strcmp (EMPI_GLOBAL_hclasses[n_classes], cmd->options[proc_increment]) == 0 && atoi(cmd->options[proc_increment+1])!=0) {
                              ...
                              // notify icc if releases nodes
                              if (EMPI_GLOBAL_nprocs_class[0][n_classes] < 0) {
                                  int num_nodes_released = (-1) * EMPI_GLOBAL_nprocs_class[0][n_classes];
                                  icc_release_register(icc, EMPI_GLOBAL_hclasses[n_classes], num_nodes_released);
                                  printf(" ICC notification: release %d nodes in compute node: %s \n",num_nodes_released,EMPI_GLOBAL_hclasses[n_classes]);
                              }
...
int command_rpc_malleability_enter_region(int procs_hint)
{
    int ret = 0;
    int rpcret = 0;
    
    ret = icc_rpc_malleability_region(icc, ICC_MALLEABILITY_REGION_ENTER, procs_hint, &rpcret);
...
int command_rpc_malleability_leave_region()
{
    ...
    ret = icc_rpc_malleability_region(icc, ICC_MALLEABILITY_REGION_LEAVE, 0, &rpcret);
    assert(ret == ICC_SUCCESS && rpcret == ICC_SUCCESS);
...
// CHANGE END
{
  ...
  // CHANGE END
  struct command_flexmpi_queue *cmd_queue;
  cmd_queue = (struct command_flexmpi_queue *)malloc(sizeof(struct command_flexmpi_queue));
  for (int i=0; i<FLEXMPI_COMMAND_QUEUE_SIZE; i++) {
      cmd_queue->cmd[i]=NULL;
  }
  cmd_queue->first_empty_pos = 0;
  icc_init_mpi(ICC_LOG_DEBUG, ICC_TYPE_FLEXMPI, nnodes, flexmpi_reconfigure, (void *)cmd_queue, &icc);
  //assert(icc != NULL);
  if (icc == NULL)
