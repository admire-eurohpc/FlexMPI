...
int MPI_Init (int *argc, char ***argv) {
    ...
    // CHANGE TEMP
    EMPI_GLOBAL_status2 = EMPI_ACTIVE;
    // END CHANGE TEMP
    ...
    if (parentcomm != MPI_COMM_NULL) {
        ...
        fprintf (stdout, "MPI_Intercomm_merge (parentcomm, 1, &EMPI_COMM_WORLD)\n");
        ...
        #endif
        fprintf (stdout, "MPI_Comm_disconnect (&parentcomm)\n");
        err = MPI_Comm_disconnect (&parentcomm);
        if (err) fprintf (stderr, "Error in MPI_Comm_disconnect\n");
        
        fprintf (stdout, "DONE MPI_Comm_disconnect (&parentcomm)\n");
        MPI_Comm_rank (EMPI_COMM_WORLD, &rank);
        MPI_Comm_size (EMPI_COMM_WORLD, &size);
        fprintf (stdout, "MPI_Init rank=%d; size=%d\n",rank,size);
        fprintf (stdout, "MPI_Barrier(EMPI_COMM_WORLD) rank=%d\n",rank);
        MPI_Barrier(EMPI_COMM_WORLD);
        fprintf (stdout, "DONE MPI_Barrier(EMPI_COMM_WORLD) rank=%d\n",rank);
    } else {
        ...
        #endif
        //Dup MPI_COMM_WORLD
        fprintf (stdout, "MPI_Comm_dup (MPI_COMM_WORLD, &EMPI_COMM_WORLD)\n");
    ...
    if (rank == EMPI_root)
    {
        ...
        // CHANGE BEGIN
        rc = pthread_create(&rpc_thread, &attr, (void*)&thread_rpc_release_nodes, NULL);
        check_posix_return(rc, "Creating thread_rpc_release_nodes");
        signal_thread_rpc_init();
        // CHANGE END
    }
    //debug
    #if (EMPI_DBGMODE > EMPI_DBG_QUIET)
...
int MPI_Finalize (void) {
    ...
    if (done == 1) {
        fprintf (stdout, "MPI_Finalize: nothing to do\n");
        return MPI_SUCCESS;
    }
    done = 1;
    ...
    if(EMPI_GLOBAL_socket!=-1){  // Only rank=0 process has a value != -1
        fprintf (stdout, "No enter (socket)\n");
...
//    fprintf (stdout, "step 5\n");
    ...
    #endif
    fprintf (stdout, "step 10 (before PMPI_Finalize)\n");
    ...
    //exit(0);
    /* CHANGE: end */
    fprintf (stdout, "step 11 (after PMPI_Finalize)\n");
