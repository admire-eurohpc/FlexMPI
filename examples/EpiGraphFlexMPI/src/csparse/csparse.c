...
*/
{
	...
	val[0]=val[1]=val[2]=0;			/* initialize val occurs */
 ...
 */
{
    int i, j, p, q, nz = 0, n, m, *Ap, *Ai, *w, *Ax ;
    int val[3]={0,0,0};
    if (!A) return (0) ;			/* check inputs */
    m = A->m ; n = A->n ; Ap = A->p ; Ai = A->i ; Ax = A->x ;
    w = cs_malloc (m, sizeof (int)) ;		/* get workspace */
    ...
    Ap [n] = nz ;				/* finalize A */
    cs_free (w) ;				/* free workspace */
    return (cs_sprealloc_int (A, 0)) ;		/* remove extra space from A */
}
...
/* C = compressed-column form of a triplet matrix T */
cs_int *cs_triplet_int (const cs_int *T, int iMyRank, FILE *flLogfile, double dTimeini)
{
    int m, n, nz, p, k, *Cp, *Ci, *w, *Ti, *Tj, *Cx, *Tx ;
    cs_int *C ;
    double dTimenow, dTimeStamp;
    if (!T) return (NULL) ;				/* check inputs */
    m = T->m ; n = T->n ; Ti = T->i ; Tj = T->p ; Tx = T->x ; nz = T->nz ;
    /* LOGFILE data */
    dTimenow = MPI_Wtime();
    dTimeStamp = dTimenow - dTimeini;
    assert ((fprintf (flLogfile, "[%.3f \t- ID_%i] CS_watchdog: cs_spalloc_int\n", dTimeStamp, iMyRank)) != EOF);
    fflush (flLogfile);
    C = cs_spalloc_int (m, n, nz, Tx != NULL, 0) ;		/* allocate result */
    /* LOGFILE data */
    dTimenow = MPI_Wtime();
    dTimeStamp = dTimenow - dTimeini;
    assert ((fprintf (flLogfile, "[%.3f \t- ID_%i] CS_watchdog: cs_calloc\n", dTimeStamp, iMyRank)) != EOF);
    fflush (flLogfile);
    w = cs_calloc (n, sizeof (int)) ;			/* get workspace */
    /* LOGFILE data */
    dTimenow = MPI_Wtime();
    dTimeStamp = dTimenow - dTimeini;
    assert ((fprintf (flLogfile, "[%.3f \t- ID_%i] CS_watchdog: cs_done_int\n", dTimeStamp, iMyRank)) != EOF);
    fflush (flLogfile);
    if (!C || !w) return (cs_done_int (C, w, NULL, 0)) ;	/* out of memory */
    Cp = C->p ; Ci = C->i ; Cx = C->x ;
    /* LOGFILE data */
    dTimenow = MPI_Wtime();
    dTimeStamp = dTimenow - dTimeini;
    assert ((fprintf (flLogfile, "[%.3f \t- ID_%i] CS_watchdog: first loop\n", dTimeStamp, iMyRank)) != EOF);
    fflush (flLogfile);
    for (k = 0 ; k < nz ; k++) w [Tj [k]]++ ;		/* column counts */
    /* LOGFILE data */
    dTimenow = MPI_Wtime();
    dTimeStamp = dTimenow - dTimeini;
    assert ((fprintf (flLogfile, "[%.3f \t- ID_%i] CS_watchdog: cs_cumsum\n", dTimeStamp, iMyRank)) != EOF);
    fflush (flLogfile);
    cs_cumsum (Cp, w, n) ;				/* column pointers */
    /* LOGFILE data */
    dTimenow = MPI_Wtime();
    dTimeStamp = dTimenow - dTimeini;
    assert ((fprintf (flLogfile, "[%.3f \t- ID_%i] CS_watchdog: second loop\n", dTimeStamp, iMyRank)) != EOF);
    fflush (flLogfile);
    for (k = 0 ; k < nz ; k++)
    {
        Ci [p = w [Tj [k]]++] = Ti [k] ;    /* A(i,j) is the pth entry in C */
    ...
    dTimenow = MPI_Wtime();
    dTimeStamp = dTimenow - dTimeini;
    assert ((fprintf (flLogfile, "[%.3f \t- ID_%i] CS_watchdog: finish\n", dTimeStamp, iMyRank)) != EOF);
    fflush (flLogfile);
    return (cs_done_int (C, w, NULL, 1)) ;	    /* success; free w and return C */
}
...
/* allocate a sparse matrix (triplet form or compressed-column form) */
cs_int *cs_spalloc_int (int m, int n, int nzmax, int values, int triplet)
{
    cs_int *A = cs_calloc (1, sizeof (cs_int)) ;    /* allocate the cs struct */
    if (!A) return (NULL) ;		    /* out of memory */
    A->m = m ;				    /* define dimensions and nzmax */
    A->n = n ;
    A->nzmax = nzmax = CS_MAX (nzmax, 1) ;
    ...
    A->p = cs_malloc (triplet ? nzmax : n+1, sizeof (int)) ;
    A->i = cs_malloc (nzmax, sizeof (int)) ;
    A->x = values ? cs_malloc (nzmax, sizeof (int)) : NULL ;
    return ((!A->p || !A->i || (values && !A->x)) ? cs_spfree_int (A) : A) ;
}

/* allocate a sparse matrix (triplet form or compressed-column form) with a number of cols*/
cs_int *cs_spalloc_int_col (int m, int n, int nzmax, int values, int triplet, int cols)
{
    cs_int *A = cs_calloc (1, sizeof (cs_int)) ;    /* allocate the cs struct */
    if (!A) return (NULL) ;		    /* out of memory */
    A->m = m ;				    /* define dimensions and nzmax */
    ...
    A->nzmax = nzmax = CS_MAX (nzmax, 1) ;
    A->nz = triplet ? 0 : -1 ;		    /* allocate triplet or comp.col */
    A->p = cs_malloc (cols+1, sizeof (int)) ;
    A->i = cs_malloc (nzmax, sizeof (int)) ;
    A->x = values ? cs_malloc (nzmax, sizeof (int)) : NULL ;
    return ((!A->p || !A->i || (values && !A->x)) ? cs_spfree_int (A) : A) ;
}
...
/* allocate a sparse matrix (triplet form or compressed-column form) */
cs_int *cs_spalloc_triplet (int m, int n, int nzmax, int values, int triplet)
{
    cs_int *A = cs_calloc (1, sizeof (cs_int)) ;    /* allocate the cs struct */
    if (!A) return (NULL) ;		    /* out of memory */
    A->m = m ;				    /* define dimensions and nzmax */
    ...
    A->nzmax = nzmax = CS_MAX (nzmax, 1) ;
    A->nz = triplet ? 0 : -1 ;		    /* allocate triplet or comp.col */
    assert ((A->p = (int*) malloc (nzmax * sizeof (int))) != NULL);
    assert ((A->i = (int*) malloc (nzmax * sizeof (int))) != NULL);
    assert ((A->x = (int*) malloc (nzmax * sizeof (int))) != NULL);
    return ((!A->p || !A->i || (values && !A->x)) ? cs_spfree_int (A) : A) ;
}
...
/* change the max # of entries sparse matrix */
int cs_sprealloc_int (cs_int *A, int nzmax)
{
    int ok, oki, okj = 1, okx = 1 ;
    if (!A) return (0) ;
    nzmax = (nzmax <= 0) ? (A->p [A->n]) : nzmax ;
    assert ((A->i = cs_realloc (A->i, nzmax, sizeof (int), &oki)) != NULL);
    if (A->nz >= 0) assert ((A->p = cs_realloc (A->p, nzmax, sizeof (int), &okj)) != NULL);
    if (A->x) assert ((A->x = cs_realloc (A->x, nzmax, sizeof (int), &okx)) != NULL);
    ok = (oki && okj && okx) ;
    if (ok) A->nzmax = nzmax ;
    return (ok) ;
}
...
/* free workspace and return a sparse matrix result */
cs_int *cs_done_int (cs_int *C, void *w, void *x, int ok)
{
    cs_free (w) ;			/* free workspace */
    cs_free (x) ;
    return (ok ? C : cs_spfree_int (C)) ;	/* return result if OK, else free it */
}
